...
/obj/machinery/power/solar/proc/updateicon()
	overlays = null
	if(stat & BROKEN)
		overlays += image('ICON/obj/power.dmi', icon_state = "solar_panel-b", layer = FLY_LAYER)
	else
		overlays += image('ICON/obj/power.dmi', icon_state = "solar_panel", layer = FLY_LAYER)
...
/obj/machinery/power/solar_control/proc/updateicon()
	...
	overlays = null
	if(cdir > 0)
		overlays += image('ICON/obj/computer.dmi', "solcon-o", FLY_LAYER, angle2dir(cdir))
	return
