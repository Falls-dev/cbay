...
/mob/living/carbon/monkey/icon = 'ICON/mob/monkey.dmi'
...
/mob/living/carbon/monkey/update_clothing()
	...
	if (wear_mask)
		if (istype(wear_mask, /obj/item/clothing/mask))
			...
			overlays += image("icon" = 'ICON/mob/monkey.dmi', "icon_state" = text("[][]", t1, (!( lying ) ? null : "2")), "layer" = layer)
		wear_mask.screen_loc = ui_mask
	if (r_hand)
		overlays += image("icon" = 'ICON/mob/items_righthand.dmi', "icon_state" = r_hand.item_state ? r_hand.item_state : r_hand.icon_state, "layer" = layer)
		r_hand.screen_loc = ui_rhand

	if (l_hand)
		overlays += image("icon" = 'ICON/mob/items_lefthand.dmi', "icon_state" = l_hand.item_state ? l_hand.item_state : l_hand.icon_state, "layer" = layer)
		l_hand.screen_loc = ui_lhand

	if (back)
		if (!( lying ))
			overlays += image("icon" = 'ICON/mob/monkey.dmi', "icon_state" = "back", "layer" = layer)
		else
			overlays += image("icon" = 'ICON/mob/monkey.dmi', "icon_state" = "back2", "layer" = layer)
		back.screen_loc = ui_back
	if (handcuffed)
		pulling = null
		if (!( lying ))
			overlays += image("icon" = 'ICON/mob/monkey.dmi', "icon_state" = "handcuff1", "layer" = layer)
		else
			overlays += image("icon" = 'ICON/mob/monkey.dmi', "icon_state" = "handcuff2", "layer" = layer)
	if (client)
