...
/obj/screen/zone_sel/MouseDown(location, control,params)		//(location, icon_x, icon_y)
	...
	overlays = null
	overlays += image("icon" = 'ICON/mob/zone_sel.dmi', "icon_state" = text("[]", selecting))
...
/mob/verb/help()
	set name = "Help"
	src << browse('ICON/help.html', "window=help")
	return
...
/*	if (client)
		...
		src << browse_rsc('ICON/88x31.png')
		src << browse('ICON/changelog.html', "window=changes;size=400x650")
		client.changes = 1*/
...
/mob/Topic(href, href_list)
	...
	if(href_list["priv_msg"])
		...
		if(M)
			...
			if (M)
				M << sound('OGGS/server/adminpm.ogg')
...
//		del(src)
	...
	for(var/obj/alertlighting/firelight/F in world)
		var/image/imagelight = image('ICON/effects/alert.dmi',F,icon_state = "blue")
		src << imagelight
	for(var/obj/alertlighting/atmoslight/G in world)
		var/image/imagelight = image('ICON/effects/alert.dmi',G,icon_state = "blueold")
		src << imagelight
...
/client/proc/station_explosion_cinematic()
	if(mob)
		...
		sleep(40)
		M << sound('OGGS/effects/explosionfar.ogg')
		boom.icon_state = "end"
		flick("explode", boom)
		sleep(40)
		if(ticker)
