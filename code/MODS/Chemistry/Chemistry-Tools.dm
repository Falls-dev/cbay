...
/obj/item/weapon/chem_grenade
	...
	attackby(obj/item/weapon/W as obj, mob/user as mob)
		if(istype(W,/obj/item/assembly/time_ignite) && !stage)
			...
			playsound(src.loc, 'OGGS/items/Screwdriver2.ogg', 25, -3)
			...
			if(beakers.len)
				...
				playsound(src.loc, 'OGGS/items/Screwdriver.ogg', 25, -3)
				name = "grenade"
				icon_state = "chemg3"
				stage = 2
			else
	...
	afterattack(atom/target as mob|obj|turf|area, mob/user as mob)
		...
		if (!src.state && stage == 2)
			...
			playsound(src.loc, 'OGGS/weapons/armbomb.ogg', 75, 1, -3)
			spawn(30)
				explode()
			user.drop_item()
	...
	proc
		explode()
			...
			if(!has_reagents)
				playsound(src.loc, 'OGGS/items/Screwdriver2.ogg', 50, 1)
				state = 0
				return

			playsound(src.loc, 'OGGS/effects/bamf.ogg', 50, 1)
...
/obj/item/weapon/gun/syringe
	...
	proc
		fire_syringe(atom/target, mob/user)
			...
			D.name = "syringe"
			playsound(user.loc, 'OGGS/effects/syringeproj.ogg', 50, 1)
			shoot:
				for(var/i=0, i<6, i++)
					if(D.loc == trg) break
