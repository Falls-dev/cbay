	...
	reagent
			...
			reaction_turf(var/turf/T, var/volume)
				...
				if(istype(T, /turf/simulated) && volume >= 3)
					...
					if(T:wet_overlay)
						...
						T:wet_overlay = null
					T:wet_overlay = image('ICON/effects/water.dmi',T,"wet_floor")
					T:overlays += T:wet_overlay
		...
		lube
			...
			reaction_turf(var/turf/T, var/volume)
				...
				if(T:wet >= 2) return
				T:wet = 2
				T:wet_overlay = image('ICON/effects/effects.dmi', "slube")
				T:overlays += T:wet_overlay
				spawn(800)
					T:wet = 0
					if(T:wet_overlay)
		...
		thermite
			...
			reaction_turf(var/turf/T, var/volume)
				...
				if(istype(T, /turf/simulated/wall))
					T:thermite = 1
					T.overlays = null
					T.overlays = image('ICON/effects/effects.dmi',icon_state = "thermite")
				return
