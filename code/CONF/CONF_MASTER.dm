	...
	for (var/T in L)
		...
		if (M.config_tag && M.enabled)
			if(!(M.config_tag in modes))		// ensure each mode is added only once
				check_diary()
				to_log ("Adding game mode [M.name] ([M.config_tag]) to configuration.")
...
/datum/configuration/proc/load(filename)
	...
	if (!text)
		check_diary()
		to_log ("No config.txt file found, setting defaults")
		src = new /datum/configuration()
		return

	check_diary()
	to_log ("Reading configuration file [filename]")
			...
			if ("probability")
				...
				if (prob_pos)
					...
					if (prob_name in config.modes)
						...
						to_log ("Unknown game mode probability configuration definition: [prob_name].")
				else
					to_log ("Incorrect probability configuration definition: [prob_name]  [prob_value].")
			else
				check_diary()
				to_log ("Unknown setting in configuration: '[name]'")
