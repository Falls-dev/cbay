	...
	if (usr.contents.Find(src) || usr.contents.Find(src.master) || (istype(src.loc, /turf) && get_dist(src, usr) <= 1))
		...
		if (href_list["mm"])
			...
			if (src.mode == 5)
				...
				if (!P.silent)
					playsound(P.loc, 'OGGS/machines/twobeep.ogg', 35, 1)
					for (var/mob/O in hearers(3, P.loc))
						O.show_message(text("\icon[P] *[P.ttone]*"))

				P.overlays = null
				P.overlays += image('ICON/obj/pda.dmi', "pda-r")
			...
			if (last_honk && world.time < last_honk + 20)
				return
			playsound(src.loc, 'OGGS/items/bikehorn.ogg', 50, 1)
		...
		if ((src.honkamt > 0) && (prob(60)))
			src.honkamt--
			playsound(src.loc, 'OGGS/items/bikehorn.ogg', 30, 1)
