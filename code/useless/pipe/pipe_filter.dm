...
/obj/machinery/pipefilter/attackby(obj/item/weapon/W, mob/user as mob)
	...
	if(istype(W, /obj/item/weapon/card/emag) && (!emagged))
		...
		src.overlays += image('ICON/obj/pipes2.dmi', "filter-spark")
...
/obj/machinery/pipefilter/proc/updateicon()
	...
	if(stat & NOPOWER)
		...
		if(emagged)	//only show if powered because presumeably its the interface that has been fried
			src.overlays += image('ICON/obj/pipes2.dmi', "filter-emag")
		if (src.f_mask & (GAS_N2O|GAS_PL))
			src.overlays += image('ICON/obj/pipes2.dmi', "filter-tox")
		if (src.f_mask & GAS_O2)
			src.overlays += image('ICON/obj/pipes2.dmi', "filter-o2")
		if (src.f_mask & GAS_N2)
			src.overlays += image('ICON/obj/pipes2.dmi', "filter-n2")
		if (src.f_mask & GAS_CO2)
			src.overlays += image('ICON/obj/pipes2.dmi', "filter-co2")
	if(!locked)
		src.overlays += image('ICON/obj/pipes2.dmi', "filter-open")
		if(bypassed)	//should only be bypassed if unlocked
			src.overlays += image('ICON/obj/pipes2.dmi', "filter-bypass")