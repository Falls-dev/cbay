...
/mob/living/carbon/human/icon = 'ICON/mob/mob.dmi'
...
/mob/living/carbon/human/New()
	...
	if(!stand_icon)
		stand_icon = new /icon('ICON/mob/human.dmi', "body_[g]_s")
	if(!lying_icon)
		lying_icon = new /icon('ICON/mob/human.dmi', "body_[g]_l")
	icon = stand_icon
			...
			if(loc:ul_Luminosity() < 3 || blinded)
				...
				if (world.time > lastDblClick+2)
					...
					if(((prob(40)) || (prob(95) && mutations & 16)) && W.status)
						...
						playsound(loc, 'OGGS/weapons/Egloves.ogg', 50, 1, -1)
						W:charges--
					else if(W.status)
...
/mob/living/carbon/human/update_clothing()
	...
	// lol
	if (mutations & 8)
		overlays += image("icon" = 'ICON/effects/genetics.dmi', "icon_state" = "hulk[!lying ? "_s" : "_l"]")

	if (mutations & 2)
		overlays += image("icon" = 'ICON/effects/genetics.dmi', "icon_state" = "fire[!lying ? "_s" : "_l"]")

	if (mutations & 1)
		overlays += image("icon" = 'ICON/effects/genetics.dmi', "icon_state" = "telekinesishead[!lying ? "_s" : "_l"]")

	if (mutantrace)
		overlays += image("icon" = 'ICON/effects/genetics.dmi', "icon_state" = "[mutantrace][!lying ? "_s" : "_l"]")
		if(face_standing)
			del(face_standing)
		if(face_lying)
			del(face_lying)
	...
	//	zone_sel.overlays += image("icon" = 'ICON/mob/zone_sel.dmi', "icon_state" = text("[]", zone_sel.selecting))
	...
	if (w_uniform)
		...
		if (istype(w_uniform, /obj/item/clothing/under))
			...
			if (!t1)
				t1 = icon_state
			/*if (mutations & 32)
				overlays += image("icon" = 'ICON/mob/uniform_fat.dmi', "icon_state" = "[t1][!lying ? "_s" : "_l"]", "layer" = MOB_LAYER)*/
			//else
			overlays += image("icon" = 'ICON/mob/uniform.dmi', "icon_state" = text("[][]",t1, (!(lying) ? "_s" : "_l")), "layer" = MOB_LAYER)
			if (w_uniform.blood_DNA)
				var/icon/stain_icon = icon('ICON/effects/blood.dmi', "uniformblood[!lying ? "" : "2"]")
				overlays += image("icon" = stain_icon, "layer" = MOB_LAYER)

	if (wear_id)
		overlays += image("icon" = 'ICON/mob/mob.dmi', "icon_state" = "id[!lying ? null : "2"]", "layer" = MOB_LAYER)
	...
	if (gloves)
		...
		overlays += image("icon" = 'ICON/mob/hands.dmi', "icon_state" = text("[][]", t1, (!( lying ) ? null : "2")), "layer" = MOB_LAYER)
		if (gloves.blood_DNA)
			var/icon/stain_icon = icon('ICON/effects/blood.dmi', "bloodyhands[!lying ? "" : "2"]")
			overlays += image("icon" = stain_icon, "layer" = MOB_LAYER)
	else if (blood_DNA)
		var/icon/stain_icon = icon('ICON/effects/blood.dmi', "bloodyhands[!lying ? "" : "2"]")
		overlays += image("icon" = stain_icon, "layer" = MOB_LAYER)
	...
	if (glasses)
		...
		overlays += image("icon" = 'ICON/mob/eyes.dmi', "icon_state" = text("[][]", t1, (!( lying ) ? null : "2")), "layer" = MOB_LAYER)
	// Ears
	if (ears)
		var/t1 = ears.icon_state
		overlays += image("icon" = 'ICON/mob/ears.dmi', "icon_state" = text("[][]", t1, (!( lying ) ? null : "2")), "layer" = MOB_LAYER)
	// Shoes
	if (shoes)
		var/t1 = shoes.icon_state
		overlays += image("icon" = 'ICON/mob/feet.dmi', "icon_state" = text("[][]", t1, (!( lying ) ? null : "2")), "layer" = MOB_LAYER)
		if (shoes.blood_DNA)
			var/icon/stain_icon = icon('ICON/effects/blood.dmi', "shoesblood[!lying ? "" : "2"]")
			overlays += image("icon" = stain_icon, "layer" = MOB_LAYER)	// Radio
/*	if (w_radio)
		overlays += image("icon" = 'ICON/mob/ears.dmi', "icon_state" = "headset[!lying ? "" : "2"]", "layer" = MOB_LAYER) */
	...
	if (wear_mask)
		if (istype(wear_mask, /obj/item/clothing/mask))
			...
			overlays += image("icon" = 'ICON/mob/mask.dmi', "icon_state" = text("[][]", t1, (!( lying ) ? null : "2")), "layer" = MOB_LAYER)
			if (!istype(wear_mask, /obj/item/clothing/mask/cigarette))
				if (wear_mask.blood_DNA)
					var/icon/stain_icon = icon('ICON/effects/blood.dmi', "maskblood[!lying ? "" : "2"]")
					overlays += image("icon" = stain_icon, "layer" = MOB_LAYER)
	...
	if (wear_suit)
		...
		if (istype(wear_suit, /obj/item/clothing/suit))
			...
			overlays += image("icon" = 'ICON/mob/suit.dmi', "icon_state" = text("[][]", t1, (!( lying ) ? null : "2")), "layer" = MOB_LAYER)
		if (wear_suit.blood_DNA)
			...
			if (istype(wear_suit, /obj/item/clothing/suit/armor/vest || /obj/item/clothing/suit/wcoat || /obj/item/clothing/suit/armor/a_i_a_ptank))
				stain_icon = icon('ICON/effects/blood.dmi', "armorblood[!lying ? "" : "2"]")
			else if (istype(wear_suit, /obj/item/clothing/suit/storage/det_suit || /obj/item/clothing/suit/storage/labcoat))
				stain_icon = icon('ICON/effects/blood.dmi', "coatblood[!lying ? "" : "2"]")
			else
				stain_icon = icon('ICON/effects/blood.dmi', "suitblood[!lying ? "" : "2"]")
			overlays += image("icon" = stain_icon, "layer" = MOB_LAYER)
	...
	if (head)
		...
		var/icon/head_icon = icon('ICON/mob/head.dmi', text("[][]", t1, (!( lying ) ? null : "2")))
		overlays += image("icon" = head_icon, "layer" = MOB_LAYER)
		if (head.blood_DNA)
			var/icon/stain_icon = icon('ICON/effects/blood.dmi', "helmetblood[!lying ? "" : "2"]")
			overlays += image("icon" = stain_icon, "layer" = MOB_LAYER)
	...
	if (belt)
		...
		if (!t1)
			t1 = belt.icon_state
		overlays += image("icon" = 'ICON/mob/belt.dmi', "icon_state" = text("[][]", t1, (!( lying ) ? null : "2")), "layer" = MOB_LAYER)
	...
	if (back)
		...
		overlays += image("icon" = 'ICON/mob/back.dmi', "icon_state" = text("[][]", t1, (!( lying ) ? null : "2")), "layer" = MOB_LAYER)
		back.screen_loc = ui_back

	if (handcuffed)
		pulling = null
		if (!lying)
			overlays += image("icon" = 'ICON/mob/mob.dmi', "icon_state" = "handcuff1", "layer" = MOB_LAYER)
		else
			overlays += image("icon" = 'ICON/mob/mob.dmi', "icon_state" = "handcuff2", "layer" = MOB_LAYER)
	...
	if (r_hand)
		overlays += image("icon" = 'ICON/mob/items_righthand.dmi', "icon_state" = r_hand.item_state ? r_hand.item_state : r_hand.icon_state, "layer" = MOB_LAYER+1)

		r_hand.screen_loc = ui_rhand

	if (l_hand)
		overlays += image("icon" = 'ICON/mob/items_lefthand.dmi', "icon_state" = l_hand.item_state ? l_hand.item_state : l_hand.icon_state, "layer" = MOB_LAYER+1)
	...
	if (shielded)
		overlays += image("icon" = 'ICON/mob/mob.dmi', "icon_state" = "shield", "layer" = MOB_LAYER)
...
/mob/living/carbon/human/attack_alien(mob/living/carbon/alien/humanoid/M as mob)
	...
	if (M.a_intent == "help")
		...
		if (M.a_intent == "grab")
			...
			grabbed_by += G
			G.synch()
			playsound(loc, 'OGGS/weapons/thudswoosh.ogg', 50, 1, -1)
			for(var/mob/O in viewers(src, null))
		...
		if (M.a_intent == "grab")
			...
			if (M.a_intent == "hurt" && !(M.gloves && M.gloves.elecgen == 1))
				...
				if ((istype(affecting, /datum/organ/external) && prob(90)))
					...
					playsound(loc, 'OGGS/weapons/punchmiss.ogg', 25, 1, -1)
					for(var/mob/O in viewers(src, null))
...
/mob/living/carbon/human/proc/update_body()
	...
	stand_icon = new /icon('ICON/mob/human.dmi', "blank")
	lying_icon = new /icon('ICON/mob/human.dmi', "blank")

	var/husk = (mutations & 64)
	//var/obese = (mutations & 32)

	if (husk)
		stand_icon.Blend(new /icon('ICON/mob/human.dmi', "husk_s"), ICON_OVERLAY)
		lying_icon.Blend(new /icon('ICON/mob/human.dmi', "husk_l"), ICON_OVERLAY)
	/*else if(obese)
		stand_icon.Blend(new /icon('ICON/mob/human.dmi', "fatbody_s"), ICON_OVERLAY)
		lying_icon.Blend(new /icon('ICON/mob/human.dmi', "fatbody_l"), ICON_OVERLAY)*/
	else
		stand_icon.Blend(new /icon('ICON/mob/human.dmi', "chest_[g]_s"), ICON_OVERLAY)
		lying_icon.Blend(new /icon('ICON/mob/human.dmi', "chest_[g]_l"), ICON_OVERLAY)
	//	if (underwear > 0)
	//		if(!obese)
		//		stand_icon.Blend(new /icon('ICON/mob/human.dmi', "underwear[underwear]_[g]_s"), ICON_OVERLAY)
	//			lying_icon.Blend(new /icon('ICON/mob/human.dmi', "underwear[underwear]_[g]_l"), ICON_OVERLAY)
		for (var/part in list("head", "arm_left", "arm_right", "hand_left", "hand_right", "leg_left", "leg_right", "foot_left", "foot_right"))
			stand_icon.Blend(new /icon('ICON/mob/human.dmi', "[part]_s"), ICON_OVERLAY)
			lying_icon.Blend(new /icon('ICON/mob/human.dmi', "[part]_l"), ICON_OVERLAY)

		stand_icon.Blend(new /icon('ICON/mob/human.dmi', "groin_[g]_s"), ICON_OVERLAY)
		lying_icon.Blend(new /icon('ICON/mob/human.dmi', "groin_[g]_l"), ICON_OVERLAY)
...
/mob/living/carbon/human/proc/update_face()
	...
	var/icon/eyes_s = new/icon("icon" = 'ICON/mob/human_face.dmi', "icon_state" = "eyes_s")
	var/icon/eyes_l = new/icon("icon" = 'ICON/mob/human_face.dmi', "icon_state" = "eyes_l")
	eyes_s.Blend(rgb(r_eyes, g_eyes, b_eyes), ICON_ADD)
	eyes_l.Blend(rgb(r_eyes, g_eyes, b_eyes), ICON_ADD)

	var/icon/hair_s = new/icon("icon" = 'ICON/mob/human_face.dmi', "icon_state" = "[hair_icon_state]_s")
	var/icon/hair_l = new/icon("icon" = 'ICON/mob/human_face.dmi', "icon_state" = "[hair_icon_state]_l")
	hair_s.Blend(rgb(r_hair, g_hair, b_hair), ICON_ADD)
	hair_l.Blend(rgb(r_hair, g_hair, b_hair), ICON_ADD)

	var/icon/facial_s = new/icon("icon" = 'ICON/mob/human_face.dmi', "icon_state" = "[face_icon_state]_s")
	var/icon/facial_l = new/icon("icon" = 'ICON/mob/human_face.dmi', "icon_state" = "[face_icon_state]_l")
	facial_s.Blend(rgb(r_facial, g_facial, b_facial), ICON_ADD)
	facial_l.Blend(rgb(r_facial, g_facial, b_facial), ICON_ADD)

	var/icon/mouth_s = new/icon("icon" = 'ICON/mob/human_face.dmi', "icon_state" = "mouth_[g]_s")
	var/icon/mouth_l = new/icon("icon" = 'ICON/mob/human_face.dmi', "icon_state" = "mouth_[g]_l")
...
/mob/living/carbon/human/UpdateDamageIcon()
	...
	for (var/datum/organ/external/O in L)
		...
		var/icon/DI = new /icon('ICON/mob/dam_human.dmi', O.damage_state)			// the damage icon for whole human
		DI.Blend(new /icon('ICON/mob/dam_mask.dmi', O.icon_name), ICON_MULTIPLY)		// mask with this organ's pixels

//		world << "[O.icon_name] [O.damage_state] \icon[DI]"

		body_standing += DI

		DI = new /icon('ICON/mob/dam_human.dmi', "[O.damage_state]-2")				// repeat for lying icons
		DI.Blend(new /icon('ICON/mob/dam_mask.dmi', "[O.icon_name]2"), ICON_MULTIPLY)
...
/proc/UpdateZombieIcons()
	spawn(0)
		for(var/mob/living/carbon/human/H in world)
			del(H.zombieimage)
			if(H.zombie)
				H.zombieimage = image('ICON/mob/mob.dmi', loc = H, icon_state = "rev")
			else if(H.zombifying)
				H.zombieimage = image('ICON/mob/mob.dmi', loc = H, icon_state = "rev_head")
			else
				H.zombieimage = null
		for(var/mob/living/carbon/human/H in world)
			if(H.zombie)
	...
	if(user in stomach_contents)
		if(prob(40))
			...
			if(I && I.force)
				...
				playsound(user.loc, 'OGGS/effects/attackblob.ogg', 50, 1)
