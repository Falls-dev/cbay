...
/obj/critter/
	...
	icon = 'ICON/GoonCritterPort/critter.dmi'
	layer = 5.0
	density = 1
	anchored = 0
	var/alive = 1
...
/obj/critter/proc/ChaseAttack(mob/M)
	...
	//playsound(src.loc, 'OGGS/weapons/genhit1.ogg', 50, 1, -1)
...
/obj/critter/yeti
	...
	ChaseAttack(mob/M)
		...
		playsound(src.loc, 'OGGS/effects/bang.ogg', 50, 1, -1)
		M.stunned = 10
		M.weakened = 10
	...
	CritterAttack(mob/M)
		...
		if(ishuman(M))
			...
			animation.icon = 'ICON/mob/mob.dmi'
...
/obj/critter/killertomato
	...
	CritterDeath()
		...
		src.alive = 0
		playsound(src.loc, 'OGGS/effects/splat.ogg', 100, 1)
...
/obj/critter/martian
	...
	attack_hand(var/mob/user as mob)
		...
		if (user.a_intent == "hurt")
			...
			for(var/mob/O in viewers(src, null))
				O.show_message("\red <b>[user]</b> punches [src]!", 1)
			playsound(src.loc, pick('OGGS/weapons/punch1.ogg','OGGS/weapons/punch2.ogg','OGGS/weapons/punch3.ogg','OGGS/weapons/punch4.ogg'), 100, 1)
			if (src.alive && src.health <= 0) src.CritterDeath()
			if (src.defensive)
				MartianPsyblast(user)
				for(var/mob/O in viewers(src, null))
...
/obj/critter/martian/proc/MartianPsyblast(mob/target as mob)
	...
	playsound(target.loc, 'OGGS/effects/ghost2.ogg', 100, 1)
	target.paralysis += 10
