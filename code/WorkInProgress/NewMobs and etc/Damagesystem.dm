...
/mob/living/carbon/human/update_clothing()
	...
	if (mutations & 8)
		overlays += image("icon" = 'ICON/effects/genetics.dmi', "icon_state" = "hulk[fat][!lying ? "_s" : "_l"]")

	if (mutations & 2)
		overlays += image("icon" = 'ICON/effects/genetics.dmi', "icon_state" = "fire[fat][!lying ? "_s" : "_l"]")

	if (mutations & 1)
		overlays += image("icon" = 'ICON/effects/genetics.dmi', "icon_state" = "telekinesishead[fat][!lying ? "_s" : "_l"]")

	if (mutantrace)
		overlays += image("icon" = 'ICON/effects/genetics.dmi', "icon_state" = "[mutantrace][fat][!lying ? "_s" : "_l"]")
		if(face_standing)
			del(face_standing)
		if(face_lying)
			del(face_lying)
	...
	if (w_uniform)
		...
		if (istype(w_uniform, /obj/item/clothing/under))
			...
			if(!lying)
				...
				overlays += image('ICON/mob/uniform.dmi',"[iconx]",MOB_LAYER)
			else
				var/iconx = "[t1]_l"
				overlays += image('ICON/mob/uniform.dmi',"[iconx]",MOB_LAYER)
	...
	if (gloves)
		...
		if(!lying)
			if(!rhand.destroyed)
				overlays += image('ICON/mob/hands.dmi',"[t1]_rhand",MOB_LAYER)
			if(!lhand.destroyed)
				overlays += image('ICON/mob/hands.dmi',"[t1]_lhand",MOB_LAYER)
		else
			if(!rhand.destroyed)
				overlays += image('ICON/mob/hands.dmi',"[t1]2_rhand",MOB_LAYER)
			if(!lhand.destroyed)
				overlays += image('ICON/mob/hands.dmi',"[t1]2_lhand",MOB_LAYER)
		if (gloves.blood_DNA)
...
/*		if(src.hair)
	...
	if (s_store)
		...
		overlays += image("icon" = 'ICON/mob/belt_mirror.dmi', "icon_state" = text("[][]", t1, (!( lying ) ? null : "2")), "layer" = MOB_LAYER)
...
/*	for(var/datum/organ/external/part in organs2)
		...
		if(istype(part,/datum/organ/external/chest))
			if (underwear > 0)
				...
				lying_icon.Blend(new /icon('ICON/mob/human.dmi', "underwear[underwear]_[g]_l"), ICON_OVERLAY)
		if(!part.destroyed)
			stand_icon.Blend(new /icon('ICON/mob/human.dmi', "[part.icon_name]_s"), ICON_OVERLAY)
			lying_icon.Blend(new /icon('ICON/mob/human.dmi', "[part.icon_name]_l"), ICON_OVERLAY)*/
