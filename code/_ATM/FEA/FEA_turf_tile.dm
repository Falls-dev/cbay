...
//				air.parent = src //TODO DEBUG REMOVE
		...
		//	if(ticker) overlayss += 'ICON/debug_update.dmi'

			if(zone)
				for(var/direction in cardinal)
					if(air_check_directions&direction)
					//	if(ticker) overlayss += image('ICON/Testing/turf_analysis.dmi',src,"arrow",dir=direction)
					...
					//	if(ticker) overlayss += image('ICON/Testing/turf_analysis.dmi',src,"red_arrow",dir=direction)
						var/turf/simulated/T = get_step(src,direction)
						if(T)
			...
			if(parent)
				...
				for(var/direction in cardinal)
					if(air_check_directions&direction)
						//(ticker) overlayss += image('ICON/Testing/turf_analysis.dmi',src,"arrow",dir=direction)
						var/turf/simulated/T = get_step(src,direction)
