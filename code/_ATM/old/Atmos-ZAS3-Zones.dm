...
zone
	...
	New(turf/start,soxy = 0,snitro = 0,sco2 = 0, spla2 = 0,smembers,sspace)//,sedges)
		...
		if(!start.CanPass(null,start,0,1)) //Bug here.
			...
			//start.overlays += 'ICON/Testing/turf_analysis.dmi'
			del src
			return 0
			...
			if(turf_plasma > MOLES_PLASMA_VISIBLE)
				if(!shows_plasma)
					...
					for(var/turf/A in members)
						A.z_image = image('ICON/effects/tile_effects.dmi',A,"plasma")
						world << A.z_image
			else
				if(shows_plasma)
					shows_plasma = 0
...
turf/proc/ZoneInfo()
	...
	for(var/turf/simulated/T)
		T.overlays -= 'ICON/debug_group.dmi'
		T.overlays -= 'ICON/debug_space.dmi'
		T.overlays -= 'ICON/debug_connect.dmi'
