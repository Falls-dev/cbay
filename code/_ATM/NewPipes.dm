...
/obj/cabling/flexipipe
	...
	simple
		...
		New()
			...
			. = ..()
		insulated
			icon = 'ICON/obj/atmospherics/red_pipe.dmi'
			icon_state = "intact"
			New()
				icon = 'ICON/obj/pipes_insulated.dmi'
				switch(dir)
					if(1 to 2) icon_state = "1-2"
					if(4) icon_state = "4-8"
					if(5) icon_state = "1-4"
...
/obj/machinery/atmos_new
	icon = 'ICON/obj/pipes_placeholder.dmi'
	anchored = 1
	proc/Process()
	proc/Detach()
	...
	*/
	...
	tank
		name = "Gas Tank"
		icon = 'ICON/obj/atmospherics/pipe_tank.dmi'
		var/volume = 1620
		...
		plasma_tank
			...
			icon = 'ICON/obj/atmospherics/orange_pipe_tank.dmi'
			New()
				. = ..()
		...
		oxy_tank
			...
			icon = 'ICON/obj/atmospherics/blue_pipe_tank.dmi'
			New()
				. = ..()
		...
		air_tank
			name = "Gas Tank \[Air\]"
			icon = 'ICON/obj/atmospherics/red_pipe_tank.dmi'
			New()
				. = ..()
	...
	filter
		...
		Process(datum/UnifiedNetworkController/FlexipipeNetworkController/Controller)
			...
			if(!A)
				...
				//A.overlays += 'ICON/debug_connect.dmi'
			if(!B)
				B = get_step(src,dir)
				//B.overlays += 'ICON/debug_connect.dmi'
			if(!C)
				C = get_step(src,turn(dir,180))
				//C.overlays += 'ICON/debug_connect.dmi'
		...
		examine()
			...
			for(var/obj/cabling/C in Network.Cables)
				C.overlays += 'ICON/debug_update.dmi'
				spawn(10) C.overlays -= 'ICON/debug_update.dmi'
	mixer
	...
	thermal_plate
		...
		proc/radiate(datum/UnifiedNetworkController/FlexipipeNetworkController/Controller)
			...
			return 1
	generator_input
		icon = 'ICON/obj/atmospherics/heat_exchanger.dmi'
		icon_state = "intact"
		density = 1
		...
		switch(Device.type)
			if(/obj/item/device/analyzer)
				...
				if (total_moles>0)
					...
					//for(var/obj/cabling/C in Network.Cables)
					//	C.overlays += 'ICON/debug_update.dmi'
					//	spawn(5) C.overlays -= 'ICON/debug_update.dmi'
				else
