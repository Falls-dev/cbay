...
proc/explosion(turf/epicenter, devastation_range, heavy_impact_range, light_impact_range, flash_range, force = 0)
	...
	spawn(0)
		...
		//playsound(epicenter.loc, 'OGGS/effects/explosionfar.ogg', 100, 1, round(devastation_range*2,1) )
		playsound(epicenter.loc, "explosion", 100, 1, round(devastation_range,1) )
		var/sound/distant_explosion = sound('OGGS/effects/explosionfar.ogg')
		for(var/mob/M)
			M << distant_explosion


		//NEW EXPLOSION CODE - NICER (3D TOO!!) BLASTS
		//epicenter.overlays += image('ICON/obj/status_display.dmi', "epicenter")
		...
		while (fillqueue.len > 0)
			...
			for(var/dir in cardinal3d) //TODO replace with "IsSameStation" checks
				...
				//T.overlays += image('ICON/obj/status_display.dmi', "black")


		del D

		//for(var/turf/T in detonate)
		//	T.overlays += image('ICON/obj/status_display.dmi', "red")
		//	var/image/I = image('ICON/obj/status_display.dmi', "[detdists[T]]")
		//	I.pixel_x = 1
		//	I.pixel_y = -1
		//	T.overlays += I
