...
/turf/simulated/wall/proc/dismantle_wall(devastated=0)
	...
	if(istype(src,/turf/simulated/wall/r_wall))
		if(!devastated)
			playsound(src.loc, 'OGGS/items/Welder.ogg', 100, 1)
...
/turf/simulated/wall/attack_hand(mob/user as mob)
	...
	playsound(src.loc, 'OGGS/weapons/Genhit.ogg', 25, 1)
...
/turf/simulated/wall/r_wall/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if (istype(W, /obj/item/weapon/weldingtool) && W:welding)
		...
		if (src.d_state == 4)
			...
			playsound(src.loc, 'OGGS/items/Ratchet.ogg', 100, 1)
			sleep(40)
			if ((user.loc == T && user.equipped() == W))
				src.d_state = 5
		...
		if (src.d_state == 0)
			playsound(src.loc, 'OGGS/items/Wirecutter.ogg', 100, 1)
			src.d_state = 1
			new /obj/item/weapon/rods( src )
		...
		if (src.d_state == 3)
			...
			playsound(src.loc, 'OGGS/items/Crowbar.ogg', 100, 1)
			sleep(100)
			if ((user.loc == T && user.equipped() == W))
				src.d_state = 4
...
/turf/simulated/floor/engine/attackby(obj/item/weapon/C as obj, mob/user as mob)
	...
	if(istype(C, /obj/item/weapon/wrench))
		...
		playsound(src.loc, 'OGGS/items/Ratchet.ogg', 80, 1)
		if(do_after(user, 30))
			new /obj/item/weapon/rods(src)
			new /obj/item/weapon/rods(src)
			ReplaceWithFloor()
...
/turf/simulated/floor/attackby(obj/item/weapon/C as obj, mob/user as mob)
	...
	if(istype(C, /obj/item/weapon/crowbar) && intact)
		...
		to_plating()
		playsound(src.loc, 'OGGS/items/Crowbar.ogg', 80, 1)
				...
				if (istype(src,/turf/simulated/floor/open))
					...
					if(do_after(user, 30))
						...
						if (C:amount <= 0)
							user.u_equip(C)
							del(C) //wtf
						playsound(src.loc, 'OGGS/items/Deconstruct.ogg', 80, 1)
			else
	...
	if(istype(C, /obj/item/weapon/tile) && !intact)
		if(istype(src,/turf/simulated/floor/open)) //Act like space if open.
			if(locate(/obj/lattice, src))
				...
				del(L)
				playsound(src.loc, 'OGGS/weapons/Genhit.ogg', 50, 1)
				C:build(src)
				C:amount--
