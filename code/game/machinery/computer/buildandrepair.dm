/obj/computerframe
	...
	anchored = 0
	name = "Computer-frame"
	icon = 'ICON/obj/computer_frame.dmi'
	icon_state = "0"
	var/state = 0
	...
	switch(state)
		if(0)
			if(istype(P, /obj/item/weapon/wrench))
				playsound(src.loc, 'OGGS/items/Ratchet.ogg', 50, 1)
				if(do_after(user, 20))
					user << "\blue You wrench the frame into place."
					src.anchored = 1
					src.state = 1
			if(istype(P, /obj/item/weapon/weldingtool))
				playsound(src.loc, 'OGGS/items/Welder.ogg', 50, 1)
				if(do_after(user, 20))
		...
		if(1)
			...
			if(istype(P, /obj/item/weapon/circuitboard) && !circuit)
				playsound(src.loc, 'OGGS/items/Deconstruct.ogg', 50, 1)
			...
			if(istype(P, /obj/item/weapon/crowbar) && circuit)
				playsound(src.loc, 'OGGS/items/Crowbar.ogg', 50, 1)
		...
		if(3)
			if(istype(P, /obj/item/weapon/wirecutters))
				playsound(src.loc, 'OGGS/items/Wirecutter.ogg', 50, 1)
