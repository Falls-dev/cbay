	...
	switch(mode)
		...
		if(SECBOT_HUNT)		// hunting for perp
			...
			if (target)		// make sure target exists
				if (get_dist(src, src.target) <= 1&&CanReachThrough(get_turf(src), get_turf(target), target))		// if right next to perp
					playsound(src.loc, 'OGGS/weapons/Egloves.ogg', 50, 1, -1)
		...
		if(SECBOT_PREP_ARREST)		// preparing to arrest target
			...
			if (!src.target.handcuffed && !src.arrest_type)
				playsound(src.loc, 'OGGS/weapons/handcuffs.ogg', 30, 1, -2)
				mode = SECBOT_ARREST
				for(var/mob/O in viewers(src, null))
				...
				spawn(60)
					if (get_dist(src, src.target) <= 1)
						...
						src.frustration = 0
						src.secure_arrest = 0
						playsound(src.loc, pick('OGGS/voice/bgod.ogg', 'OGGS/voice/biamthelaw.ogg', 'OGGS/voice/bsecureday.ogg', 'OGGS/voice/bradio.ogg', 'OGGS/voice/binsult.ogg', 'OGGS/voice/bcreep.ogg'), 50, 0)
...
/obj/machinery/bot/secbot/proc/look_for_perp()
	...
	for (var/mob/living/carbon/C in view(7,src)) //Let's find us a criminal
		...
		//	playsound(src.loc, pick('OGGS/voice/bcriminal.ogg', 'OGGS/voice/bjustice.ogg', 'OGGS/voice/bfreeze.ogg'), 50, 0)
...
/obj/machinery/bot/secbot/proc/explode()
	...
	Sa.build_step = 1
	Sa.overlays += image('ICON/obj/aibots.dmi', "hs_hole")
...
/obj/item/weapon/secbot_assembly/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if ((istype(W, /obj/item/weapon/weldingtool)) && (!src.build_step))
		if ((W:welding) && (W:get_fuel() >= 1))
			W:use_fuel(1)
			src.build_step++
			src.overlays += image('ICON/obj/aibots.dmi', "hs_hole")
		...
		src.overlays += image('ICON/obj/aibots.dmi', "hs_eye")
		...
		src.overlays += image('ICON/obj/aibots.dmi', "hs_arm")
		del(W)
